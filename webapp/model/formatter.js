sap.ui.define([
	...
	var formatter = {
		...
		catCounter:function(sCat, aProducts) {
			return aProducts.filter(function(oProduct) {
				if (!oProduct.category) {
					return false;
				}
				return oProduct.category._id === sCat;
			}).length;
		},
		/**
		 ...
		 */
		statusState: function (bStatus) {
			return bStatus ? "Success" : "Error";
		},
		...
		pictureUrl: function (sUrl) {
			if (sUrl){
				return  "http://127.0.0.1:8080/api" + sUrl;
			} else {
				return undefined;
			}
		},
